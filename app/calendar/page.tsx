...
import { CalendarClock, Check, ChevronLeft, ChevronRight, Pencil, Trash2 } from "lucide-react";
import { clsx } from "clsx";
import { AppShell } from "@/components/app-shell";
import { Card, Section } from "@/components/ui";
import { currency, demoMonth } from "@/lib/calculations";
...
export default function CalendarPage() {
  const { payments, addPayment, updatePayment, removePayment } = useStore();
  ...
  const monthlyTotal = payments.reduce((total, p) => total + p.amountMxn, 0);
  const dayPayments = payments
    .filter((p) => p.dayOfMonth === selectedDay)
  ...
  return (
    <AppShell
      title="Calendar"
      subtitle={`${currency.format(monthlyTotal)} in scheduled payments each month`}
    >
      <Card className="mb-4 flex items-center gap-3 border-sky-500/25 bg-sky-500/[0.07] dark:bg-sky-500/10">
        ...
        <p className="text-sm leading-5 text-ink/60 dark:text-cloud/60">
          Payments repeat monthly on their day. Anything still due this month is reserved
          before your safe-to-spend is calculated.
        </p>
      </Card>
      ...
      <Card className="p-4">
        ...
        <div className="mt-3 grid grid-cols-7 text-center">
          ...
          {Array.from({ length: daysInViewedMonth }, (_, index) => index + 1).map((day) => {
            const hasPayments = payments.some((p) => p.dayOfMonth === day);
                ...
                {hasPayments ? (
                  <span
                    className={clsx(
                      ...
                      isSelected ? "bg-emerald-400" : "bg-sky-500"
                    )}
                  />
                ) : null}
              </button>
